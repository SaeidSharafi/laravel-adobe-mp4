<template>
  <div class="bg-white dark:bg-slate-800 flex items-start w-full rounded-lg p-6 h-full card">
    <component
        ...
        >
            <div class="card-body">
                <div class="card-title flex items-start justify-between mb-2">
                    <div class="avatar flex-shrink-0">
                        <BaseIcon :class="computedColor" :path="icon" :size="computedSize" :h="computedHeight"
                                  :w="computedWidth"/>
                    </div>
                </div>
                <span class="block text-2xl">{{ title }}</span>
                <h3 class="text-3xl mb-1">{{ value }}<span style="font-size: 14px">{{ unitString }}</span></h3>
