        ...
        >
            <div class="sm:px-6 lg:px-8">
                <slot
                    ...
                    :on-data-fetched="changeFilterlabel"
                >
                    <TableFilter
            ...
            <div class="flex flex-row flex-wrap sm:flex-nowrap justify-start px-4 sm:px-6 lg:px-8">
                <div
                    v-if="queryBuilderProps.globalSearch"
                    class="flex flex-row w-full sm:w-auto sm:grow order-1 sm:order-2 mb-2 sm:mb-0 ltr:sm:mr-4 rtl:sm:ml-4"
                >
                    <slot
                ...
                >
                    <div
                        v-if="canBeReset"
                        class="order-5 sm:order-5 ltr:sm:mr-4 rtl:ml-4"
                    >
                ...
                >
                    <TableAddSearchRow
                        ...
                        class="order-4 sm:order-4 ltr:mr-2 rtl:ml-2 ltr:sm:mr-4 rtl:sm:ml-4"
                        :search-inputs="queryBuilderProps.searchInputsWithoutGlobal"
                        :has-search-inputs-without-value="queryBuilderProps.hasSearchInputsWithoutValue"
                        :on-add="showSearchInput"
                    />
                ...
                >
                    <TableColumns
                        ...
                        class="order-4 ltr:mr-4 rtl:ml-4 sm:mr-0 sm:order-4"
                        :columns="queryBuilderProps.columns"
                        :has-hidden-columns="queryBuilderProps.hasHiddenColumns"
                        :on-change="changeColumnStatus"
                    />
                ...
                <div v-if="exportUrl"
                     class="order-last ltr:ml-auto rtl:mr-auto">
                        ...
                        <table class="min-w-full">
                            ...
                            >
                                <tr class="!border-b border-gray-300">
                                    ...
                                    <template v-for="column in queryBuilderProps.columns">
                                        <th v-if="column.selectable" v-show="!column.hidden" class="w-5 select-all">
                                            <div class="w-full px-2">
                                                <div class="flex flex-row items-center">
                                                    <BaseButton small type="button" @click="selectAllItems"
                            ...
                            >
                                <tr
                                    ...
                                    :class="rowClass(key,item.rowClass)"
                                ...
                                >
                                    <td
                                        ...
                                        class="text-sm py-4 "
                                        :class="[column.selectable ? 'w-5 px-2' : ' px-4',
                                        column.key === 'note' ? 'whitespace-pre-line' : 'whitespace-nowrap']"
                                        :data-label="column.selectable ? '': column.translation"
                                    >
                                        <slot
                                            :name="`cell(${column.key})`"
                                            :item="item"
                                        >
                                            <label class="flex items-center" v-if="column.selectable">
                                                <Checkbox :name="column.key + '[]'" :input-value="item[column.key]"
                                                          class="m-auto"
...
import {mdiCheckboxMultipleMarked, mdiMicrosoftExcel} from "@mdi/js"
...
import TableActiveFilters from "@/Components/DataTable/TableActiveFilters.vue";
...
const updateCheckedItems = (value, e) => {

    // checkedDepartmetns.value[e] = value;
    if (value && !checkedItems.value.includes(e)) {
        checkedItems.value.push(e);
        emit('itemChecked', checkedItems.value);
        return;
    }

    checkedItems.value = checkedItems.value.filter(data => data !== e);
...
const selectAllItems = () => {
    let all_ids = resourceData.value.map((value) => {
        return value.id
    });

    checkedItems.value = all_ids.filter(data => !checkedItems.value.includes(data));
    // console.log(all_ids, checkedItems.value);
    emit('itemChecked', checkedItems.value);
}

function rowClass(key, rowClass = "") {
    return [props.striped ? "hover:bg-gray-300" : "hover:bg-gray-200", rowClass];
}
...
function changeFilterValue(key, value, force = false, label = null) {

    const intKey = findDataKey("filters", key);
    // console.log(queryBuilderData.value.filters);
    queryBuilderData.value.filters[intKey].value = value;
    filterlabels.value[key] = label;
    queryBuilderData.value.cursor = null;
    queryBuilderData.value.page = 1;
    if (force) {
        queryBuilderData.value.forced = value;
    }
}

function changeFilterlabel(key, value, label = null) {
    filterlabels.value[key] = label;
}
...
watch(queryBuilderData, () => {
    if (location.search === "?" + generateNewQueryString()) {
        return;
    }
...
const exportUrl = computed(() => {
    if (props.exportRoute) {
        return props.exportRoute + "?" + generateNewQueryString()
    }
